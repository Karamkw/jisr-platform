...
class CompanyHomeController extends GetxController {
  ...
  final Rxn<CompanyHomeModel> homeData =
      Rxn<CompanyHomeModel>();
  ...
  Future<void> fetchCompanyHome() async {
    try {
      ...
      final result =
          await _homeService.getCompanyHome();
 ...
 Future<void>
    onActiveOpportunitiesPressed() async {
  final mainController =
      Get.find<CompanyMainController>();

  final opportunitiesController =
      Get.find<CompanyOpportunitiesController>();

  // يبدأ تجهيز فلتر المهام المنشورة أولاً.
  final refreshFuture =
      opportunitiesController
          .showPublishedTasks();

  // الانتقال إلى تبويب المهام والفرص.
  mainController.changeTab(
    CompanyMainController
        .opportunitiesPageIndex,
  );
  ...
  Future<void>
      onNewApplicantsPressed() async {
    await _openOpportunitiesTab(
      type: CompanyOpportunityTypeFilter.task,
    );
  }

  Future<void>
      onPendingReviewsPressed() async {
  ...
  Future<void>
      onActiveAssignmentsPressed() async {
    await _openAssignments(
      initialStatus: 'working',
    );
  }

  Future<void> _openOpportunitiesTab({
    ...
    try {
      ...
      opportunitiesController.applyHomePreset(
        type: type,
        status:
      ...
      await opportunitiesController.fetchItems();
    } catch (_) {
      Get.snackbar(
        'تنبيه',
        'تعذر فتح قسم الفرص حاليًا',
  ...
  Future<void> _openAssignments({
    ...
    try {
      ...
      Get.snackbar(
        'تنبيه',
        'تعذر فتح المهام قيد المتابعة حاليًا',
        snackPosition: SnackPosition.BOTTOM,
      );
    }
  }
  ...
  Future<void>
      onCreateOpportunityPressed() async {
    final shouldRefresh = await Get.toNamed(
  ...
  Future<void> onRequiredActionPressed(
    ...
    switch (action.targetType) {
      ...
      case 'task_submission':
        await _openSubmittedTaskWorkspace(
          action,
        );
        break;
  ...
  Future<void> _openSubmittedTaskWorkspace(
    ...
    final assignment =
        await _findAssignmentForSubmissionAction(
      action,
    );
  ...
  Future<CompanyTaskAssignmentModel?>
      _findAssignmentForSubmissionAction(
    CompanyRequiredAction action,
  ) async {
    final taskId = action.taskId ?? 0;

    final studentUserId =
        action.studentUserId ?? 0;
    ...
    try {
      ...
      final submittedMatch =
          _firstMatchingAssignment(
        assignments,
        taskId: taskId,
  ...
  Future<void> onRecentActivityPressed(
